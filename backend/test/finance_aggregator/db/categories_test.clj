...
(deftest promote-to-header-clears-assignments-test
  (testing "creating the first child clears transactions and splits on the new parent"
    (let [parent (categories/create! setup/*test-conn* {:category/name "Food"
                                                        :category/type :expense
                                                        :category/ident :category/food})
          parent-id (:db/id parent)]
      ;; A whole-transaction assignment and a split part both point at the
      ;; soon-to-be header.
      (d/transact! setup/*test-conn* [{:transaction/external-id "tx-promote-1"
                                       :transaction/amount -10.00M
                                       :transaction/category parent-id}])
      (d/transact! setup/*test-conn* [{:transaction/external-id "tx-promote-2"
                                       :transaction/amount -20.00M
                                       :transaction/splits [{:split/amount -20.00M
                                                             :split/category parent-id
                                                             :split/order 0}]}])
      ;; Promote the parent by giving it a child.
      (categories/create! setup/*test-conn* {:category/name "Groceries"
                                             :category/type :expense
                                             :category/ident :category/groceries
                                             :category/parent parent-id})
      (let [db (d/db setup/*test-conn*)
            tx (d/pull db '[{:transaction/category [:db/id]}] [:transaction/external-id "tx-promote-1"])]
        (is (nil? (:transaction/category tx)) "the transaction is reset to uncategorized")
        (is (nil? (d/q '[:find ?c . :where [_ :split/category ?c]] db))
            "the split is reset to uncategorized")
        (is (false? (categories/in-use? setup/*test-conn* parent-id))
            "nothing references the header any more"))))

  (testing "setting a parent via update clears assignments to that parent"
    (let [parent (categories/create! setup/*test-conn* {:category/name "Transport"
                                                        :category/type :expense
                                                        :category/ident :category/transport})
          child (categories/create! setup/*test-conn* {:category/name "Gas"
                                                       :category/type :expense
                                                       :category/ident :category/gas})
          parent-id (:db/id parent)]
      (d/transact! setup/*test-conn* [{:transaction/external-id "tx-promote-3"
                                       :transaction/amount -30.00M
                                       :transaction/category parent-id}])
      (categories/update! setup/*test-conn* (:db/id child) {:category/parent parent-id})
      (let [tx (d/pull (d/db setup/*test-conn*) '[{:transaction/category [:db/id]}]
                       [:transaction/external-id "tx-promote-3"])]
        (is (nil? (:transaction/category tx))
            "the transaction assigned to the new header is cleared")))))
