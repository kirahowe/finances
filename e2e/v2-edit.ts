// Real-Chromium proof for Phase-R2 cp2: server-confirmed reconciled edit + lingering + undo.
// Reconciling a row in To-reconcile scope persists server-side; instead of vanishing, the row
...
const unreconciled = () => page.locator('#count-unreconciled').innerText();
const undoBtn = () => page.getByRole('button', { name: 'Undo' });
const redoBtn = () => page.getByRole('button', { name: 'Redo' });

// Start in To-reconcile scope so a reconciled row would normally drop out.
...
check('to-reconcile: 10 rows, count 10', (await rows()) === 10 && (await unreconciled()).trim() === '10',
  `rows=${await rows()} count=${await unreconciled()}`);
...
await page.locator('#tx-tbody tr').first().locator('input.reconciled-checkbox').check();
...
check('unreconciled count dropped to 9', (await unreconciled()).trim() === '9', `count=${await unreconciled()}`);
check('undo button now enabled, with label', !(await undoBtn().isDisabled()) &&
  ((await undoBtn().getAttribute('title')) ?? '').includes('Marked reconciled'),
...
check('undo restored count to 10', (await unreconciled()).trim() === '10', `count=${await unreconciled()}`);
...
await page.waitForFunction(() => document.querySelectorAll('#tx-tbody tr.is-stale').length === 0, null, { timeout: 5000 }).catch(() => {});
check('Cmd/Ctrl+Z undoes', (await staleRows()) === 0 && (await unreconciled()).trim() === '10',
  `stale=${await staleRows()} count=${await unreconciled()}`);
